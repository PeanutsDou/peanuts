...
def call_llm_normalize(subjects, config):
    ...
    prompt = f"""
你是一位资深游戏测试工程师。请对以下玩家反馈的 Bug 描述进行**规范化重写**和**结构化提取**。

**规范化原则：**
1. **要素齐全**：重写后的描述必须包含明确的【主体】（谁/什么东西）、【位置】（在哪发生）、【现象】（发生了什么具体问题）。
2. **去情绪化**：去除所有情绪化表达（如“垃圾”、“恶心”、“无语”），只保留事实描述。
3. **保持原意**：如果原描述已经符合上述要求，则**不要修改**。
4. **多问题拆分**：如果一条反馈包含两个或多个独立的问题，请将其**拆分**为多条独立的规范化描述。
5. **适度补全**：如果原描述简短且缺失信息（如只说“卡了”），尽量根据语境规范化为“游戏中出现卡顿现象”，但**严禁臆测**不存在的细节。
6. **结构化提取**：对于每一条规范化后的描述，请提取出三个核心要素：
   - **subject_core**: 核心主体（如“角色”、“任务”、“界面”、“道具名”）。
   - **location**: Bug发生的位置或界面（如“主城”、“副本”、“设置界面”）。
   - **phenomenon**: 具体现象（如“闪退”、“卡死”、“显示错误”）。

**格式要求**：请直接返回一个 JSON 列表，列表中的每个元素是一个对象，包含：
   - "id": 对应输入列表的序号（1, 2, ...）
   - "normalized": 一个对象列表。如果拆分了，列表中会有多个对象；没拆分就是一个。每个对象包含：
     - "text": 规范化后的完整描述字符串。
     - "core": 核心主体。
     - "loc": 位置。
     - "phen": 现象。
...
"""
    ...
    for attempt in range(max_retries):
        try:
            ...
            # print(f"LLM 规范化调用失败 (尝试 {attempt+1}): {e}")
